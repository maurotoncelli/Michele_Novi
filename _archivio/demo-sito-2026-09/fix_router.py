        ...
        tailwind.config = {
            theme: {
                extend: {
                    ...
                    fontFamily: {
                        serif: ['Newsreader', 'serif'], sans: ['Manrope', 'sans-serif'], mono: ['JetBrains Mono', 'monospace']
                    }
                }
            }
        }
...
<body class="selection:bg-medical-accent selection:text-white">
    ...
    <!-- SCRIPT CORRETTO PER IL ROUTING -->
    <script>
        // Gestione caricamento (per hero animazione)
        ...
        function navigate() {
            ...
            // Mappa fallback
            let targetViewId = 'view-' + hash;
            
            // Se la view non esiste nel DOM, fallback (es: per note-cliniche o /spalla se non implementate)
            if (!document.getElementById(targetViewId)) {
                if (hash.startsWith('patologie')) targetViewId = 'view-patologie';
                else if (hash.startsWith('sedi')) targetViewId = 'view-sedi';
            ...
            views.forEach(view => {
                if (view.id === targetViewId) {
                    view.style.display = 'block'; // Forziamo il display prima di aggiungere la classe per sicurezza
                    // Piccola pausa per permettere al browser di registrare il display: block prima dell'animazione
                    setTimeout(() => {
            ...
            // Se torniamo in home, ricarichiamo animazione varco
        ...
        // Esegui al caricamento per gestire direct links come #chi-sono
        navigate();
    </script>
</body>
</html>
...
print("File corretto. Router sistemato.")
