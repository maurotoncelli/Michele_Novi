...
import type { VoceApprofondimenti } from "@/lib/content";
...
export function ListaApprofondimenti({
  voci,
  locale,
  tags,
  limitePaper = LIMITE_PAPER,
}: {
  voci: VoceApprofondimenti[];
  locale: Locale;
  ...
  let visti = 0;
  const preview: VoceApprofondimenti[] = [];
  const resto: VoceApprofondimenti[] = [];
  for (const v of voci) {
    if (v.tipo === "paper") {
      visti += 1;
  ...
  const card = (v: VoceApprofondimenti, i: number) => (
    ...
    <div className="contenitore py-10">
      {tags && tags.length > 0 && (
        <Reveal className="mb-8 flex flex-wrap items-center gap-2">
          <span className="eyebrow mr-1">{m.approfondimenti.tag}</span>
          {tags.map((t) => (
